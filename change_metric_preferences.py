        ...
        for user in usernames:
            ...
            if self.browser.current_url == LOGIN_URL:
                ...
                try:
                    ...
                    t.sleep(random.random() + random.randint(4,6))
                    self.browser.find_element_by_id('contents').find_element_by_id('submit-button').submit() # SAVE
                    t.sleep(random.random() + random.randint(0,6))
                except:
                    log(f'FAILED! {user}, {self.browser.current_url}', id=self.id)
    ...
    def start(self):
        for user in usernames:
            ...
            if self.browser.current_url == LOGIN_URL:
                ...
                if 'Kilometers and Kilograms' not in preferences[0]:
                    log(f"WRONG METRICS (KGs & KMs) - {user}", 'WARNING', id=self.id)
                    continue

                # confirm metrics: Celsius
                if 'Celsius' not in preferences[1]:
